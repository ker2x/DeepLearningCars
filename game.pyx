...
from primitives import Rect as Rectangle, Text
...
cdef class AutoSimulation:
    cdef public list autos, events
    cdef public int generation
    cdef public double dt, learning_rate, start_time
    cdef public object top_auto, viewer, mouse, track, text
    #cdef public bool slow_car_removed
    ...
    def generateTrack(self, difficulty):
        ...
        self.track.generateTrack(Rectangle((size, size), (-size / 2.0, -size / 2.0)), width)
    ...
    def render(self):
        ...
        self.top_auto.draw()
        self.top_auto.brain.draw(Rectangle((300, 200), (self.viewer.pos.x + 15, self.viewer.pos.y + 5)))
        self.track.draw()
        self.mouse.draw()
...
cdef class Game:
    cdef public object world, t
    cdef public int fps, running
